...
//
//  class template bp_tree_array_acc represents a general purpose container
//  that supports interfaces of the C++03 sequences and associative containers.
//  A bp_tree_array_acc implements a partition of an array into a list of sub-arrays,
//  thus, the container elements are not stored in contiguous memory.
//  A bp_tree_array_acc can be used to store the elements with multiple or
//  unique keys in the order specified by a comparison object.
//  A bp_tree_array_acc supports:
...
//  - the efficient member function accumulate()
//    that has logarithmic complexity in the worst case.
//
template
<
    class _Ty_Key ,
    ...
    class _Alloc
>
class bp_tree_array_acc
{
public:
    //  types
    typedef bp_tree_array_acc < _Ty_Key , _Ty_Map , _Ty_Val ,
...
public:
    ...
    class _iter_base
    {
    friend class bp_tree_array_acc < _Ty_Key , _Ty_Map , _Ty_Val ,
                                     _KeyOfV , _MapOfV , _Pred   , _Alloc  > ;
    ...
    bp_tree_array_acc  ( const _Pred &     pred ,
                         bool              mul  ,
                         bool              ord  ,
                         const _Alloc &    alr  ) ;
    bp_tree_array_acc  ( const this_type & that ) ;
    this_type &
...
protected:
   ...
   _NodeHeavyPtr    _create_node_heavy ( _NodeLightPtr     p_light  ) ;
    ...
    void     _fill_value_count       ( size_type          cnt      ,
                                       const value_type & val      ) ;
    _Ty_Map  _sub_sum_count          ( size_type          count    ,
    ...
    void _push_back_array ( _InpIter  pos_a, _InpIter  pos_b )
    {
        ...
        while ( pos_a != pos_b )
        {
            ...
            _insert_b_tree_count ( _bottom_end()->p_prev , cnt_elem , sum_elem ) ;
        ...
        if ( cnt_node == 1 )
        {
            ...
            _bottom_end()->p_prev->m_subsum = sum_elem ;
        }
    ...
    {
        std::vector<value_type>             vect_copy ( this->begin() ,
                                                        this->end()   ) ;
        this->clear() ;
        typename
        std::vector<value_type>::iterator   end_res ;
        end_res = std::remove_if ( vect_copy.begin() , vect_copy.end() , pred ) ;
        this->_push_back_array ( vect_copy.begin() , end_res ) ;
    }
    ...
    void _unique_pred ( _PredRem  pred )
    {
        ...
        typename
        std::vector<value_type>::iterator   end_res ;
        end_res = std::unique ( vect_copy.begin() , vect_copy.end() , pred ) ;
        this->_push_back_array ( vect_copy.begin() , end_res ) ;
    }
...
#define BP_TREE_TY  bp_tree_array_acc < _Ty_Key , _Ty_Map , _Ty_Val , \
                                        _KeyOfV , _MapOfV , _Pred   , _Alloc >
...
BP_TREE_TY::_create_node_heavy ( _NodeLightPtr  p_light )
{
    ...
    try
    {
        ...
        mapped_type *   p_subsum  = &(p_n->m_subsum) ;
        ::new (p_subsum) mapped_type() ;
    }
    catch (...)
    {
...
void BP_TREE_TY::_delete_node_heavy ( _NodeHeavyPtr &  p_node )
{
    if ( p_node )
    {
        mapped_type *   p_subsum  = &(p_node->m_subsum) ;
        p_subsum->~mapped_type() ;
...
TEMPL_DECL
BP_TREE_TY::bp_tree_array_acc ( const _Pred &   pred ,
                                bool            mul  ,
                                bool            ord  ,
                                const _Alloc &  alr  ) :
...
TEMPL_DECL
BP_TREE_TY::bp_tree_array_acc ( const BP_TREE_TY &  that ) :
                                ...
                                _NodeLightPtr & p_lt_pos_upd )
{
    ...
    try
    {
        for ( i_moved = 0 ; i_moved < sz_right ; ++i_moved, ++p_lt_left, ++p_lt_right )
        {
            ...
            sum_right += _MapOfV()( p_lt_left->_elem() ) ;
    ...
    p_parent ->m_subsum -= sum_right ;
    p_par_new->m_subsum  = sum_right ;
    ...
    )
{
    ...
    {
        ++( p_tmp->m_subsz ) ;
        p_tmp->m_subsum += _MapOfV()( p_lt_elem_new->_elem() ) ;
        p_tmp = p_tmp->p_predr ;
    }
...
BP_TREE_TY::_erase_bp_tree ( iterator  pos )
{
    ...
    _Ty_Map             val_erase   = _MapOfV()( p_lt_pos_erase->_elem() ) ;
    ...
    )
{
    ...
    if ( n_move > 0 )
    {
        ...
        while ( n_mv > 0 )
        {
            ...
            ++(p_left->m_subsz) ;
            sub_sum += _MapOfV()( p_lt_0_right->_elem() ) ;
    ...
    else
    {
        ...
        while ( n_mv > 0 )
        {
            ...
            ++(p_right->m_subsz) ;
            sub_sum += _MapOfV()( (p_lt_n_left-1)->_elem() ) ;
...
TEMPL_DECL
_Ty_Map BP_TREE_TY::_sub_sum_count ( size_type           count ,
                                     const value_type &  val   ) const
{
    _Ty_Map         sum_res = _Ty_Map ( ) ;
                                    ...
                                    const value_type &  val )
{
    ...
    if ( cnt < _max_degree_ext() )
    {
        ...
        sub_sum = _sub_sum_count ( cnt , val ) ;
        _insert_b_tree_count ( _bottom_end()->p_prev , cnt , sub_sum ) ;
        return ;
    }
    ...
    while ( cnt > 2*_max_degree_ext() )
    {
        ...
        sub_sum = _sub_sum_count ( cnt_fill , val ) ;
        _insert_b_tree_count ( _bottom_end()->p_prev , cnt_fill , sub_sum ) ;
        _insert_link_node_botm ( _bottom_end() ) ;
        cnt = cnt - cnt_fill ;
    }
                         ...
                         mapped_type     val_in   ) const
{
    ...
    _NodeLightPtr   p_lt_cur   = it_start._lt_pointer() ;
    _NodeHeavyPtr   p_cur      = 0 ;

    if ( dist_limit < _max_degree_ext() )
    {
        while ( dist_cur < dist_limit )
        {
            ++dist_cur ;
            sum_res += _MapOfV()( p_lt_cur->_elem() ) ;
            _inc_pointer ( p_lt_cur ) ;
        }
        return sum_res ;
    }

    while ( p_lt_cur->p_heavy_predr == 0 )
    {
        ++dist_cur ;
        sum_res += _MapOfV()( p_lt_cur->_elem() ) ;
        ++p_lt_cur ;
    ...
    p_cur = p_lt_cur->p_heavy_predr->p_next ;
    while ( dist_cur < dist_limit )
    {
...
void BP_TREE_TY::write_shallow ( iterator             pos     ,
                                 const mapped_type &  val_new )
{
    _NodeLightPtr           p_lt_cur = pos._lt_pointer() ;
    const mapped_type &     val_old  = _MapOfV()( p_lt_cur->_elem() ) ;
    _NodeHeavyPtr           p_parent = _parent ( p_lt_cur ) ;
    ...
    mapped_type &   ref_map  = const_cast<mapped_type&>( _MapOfV()(p_lt_cur->_elem()) ) ;
    ref_map = val_new ;
}
...
void BP_TREE_TY::_splice_tree ( BP_TREE_TY &  that )
{
    ...
    if ( p_left )
    {
        ...
        if ( sz_1+sz_2 <= _max_degree_ext() )
        {
            ...
            _merge_node_with ( p_left , sub_sz , subsum ) ;
        }
        else
        {
            _NodeLightPtr   p_lt_upd = 0 ;
